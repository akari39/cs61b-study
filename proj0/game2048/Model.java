        ...
        for (int col = 0; col < board.size(); col++) {
            ...
            for (int row = board.size() - 1; row >= 0; row--) {
                ...
                if (previousTileIndex != -1) {
                    final Tile previousTile = board.tile(col, previousTileIndex);
                    if (tile.value() == previousTile.value()) {
                        board.move(col, previousTileIndex, tile);
                        score += 2 * tile.value();
                        changed = true;
                        merge += 1;
                        previousTileIndex = -1;
                ...
                final int newRow = row + vacant + merge;
                if (newRow != row) {
                    board.move(col, newRow, tile);
                    changed = true;
                }
                previousTileIndex = newRow;
            }
        }
